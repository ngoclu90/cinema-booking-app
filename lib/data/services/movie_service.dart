...
class MovieService {
  ...
  Future<ApiResponse<List<MoviePublicDto>>?> getAllMovie({
    ...
    try {
      ...
      final response = await _apiClient.dio.get(
        '${ApiEndpoints.publicBase}/movies',
      ...
      if (response.statusCode == 200) {
        ...
        final movies = rawList
            .map(
              (item) => MoviePublicDto.fromJson(item as Map<String, dynamic>),
            )
            .toList();
  ...
  Future<ApiResponse<MoviePublicDto>?> getMovieDetail(int id) async {
    try {
      final response = await _apiClient.dio.get(
        '${ApiEndpoints.publicBase}/movie-detail/$id',
      );
      if (response.statusCode == 200) {
        final responseMap = response.data as Map<String, dynamic>;
  ...
  Future<ApiResponse<List<MovieWithShowtimesDto>>?> getCinemasWithShowtimes(
    ...
    try {
      ...
      final response = await _apiClient.dio.get(
        '${ApiEndpoints.publicBase}/$movieId/cinemas-showtimes',
        queryParameters: queryParameters,
      );
      ...
      if (response.statusCode == 200) {
        ...
        final data = rawList
            .map(
              (item) =>
                  MovieWithShowtimesDto.fromJson(item as Map<String, dynamic>),
            )
            .toList();
  ...
  Future<ApiResponse<List<MovieCardDto>>?> getMoviesByStatus() async {
    try {
      final response = await _apiClient.dio.get(
        '${ApiEndpoints.publicBase}/movies/status',
      );
      if (response.statusCode == 200) {
        final responseMap = response.data as Map<String, dynamic>;
        final rawList = responseMap['data'] as List<dynamic>? ?? [];
